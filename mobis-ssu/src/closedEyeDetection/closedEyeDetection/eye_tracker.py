...
def main(args=None):
    ...
    # def nothing(x):
    #     pass
    # cv2.createTrackbar('threshold', 'image', 105, 255, nothing)
    ...
    #         eyes = cv2.bitwise_and(img, img, mask=mask)
    #         dark_pixels_mask = cv2.cvtColor(eyes, cv2.COLOR_BGR2GRAY) < dark_color_threshold
    ...
    #         eyeball_pos_left = contouring(thresh[:, 0:mid], mid, img, end_points_left)
    #         eyeball_pos_right = contouring(thresh[:, mid:], mid, img, end_points_right, True)
    #         left_close = cal_pupil_ratio(thresh, end_points_left, dark_color_threshold)
    #         right_close = cal_pupil_ratio(thresh, end_points_right, dark_color_threshold)
    #         #####
    #         # print_eye_pos(img, eyeball_pos_left, eyeball_pos_right)
    #         # eye_closed_publisher.publish_eye_position(estimate_eye_pos(img, eyeball_pos_left, eyeball_pos_right))
    #         eye_closed_publisher.publish_eye_position(judge_closedEye(left_close, right_close))
